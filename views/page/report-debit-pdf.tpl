<?php		
  	...
  	$total_sub = substr(number_format($Total, 2, '.', ','), -2);
  	if ($total_sub == 00 ) {
  	    define("MAJOR", '');
  	}else{
  		define("MAJOR", 'and cent');
  	}
		...
		$credit .= '<table>';
			...
			if ($comp == 'SVO') {
				...
				<tr>
			      	...
			      	<a class="navbar-brand"><img  src="./assets/_images/logonew/DSC_PUBLIC.jpg" style="padding-left:-10;padding-top:-20;height:85px;" /></a>
			      	</td>
			    </tr>';
			}else if($comp == 'DSL'){
				$credit .= '
	...
	function txtPage($datajsonT){
		...
		$txtpage = 
			'<table>
				<tr>
				    <td align="left">
				        <font size="1"><i>CreateBy : '.$cb.'</i></font>
				    </td>
				    <td align="right">
				        <font size="1"><i>page : {PAGENO}/{nb}</i></font>
				    </td>
				</tr>
			</table>';
		return $txtpage;
	...
	function txtGrandTotolSig($datajsonT,$datajsonL,$Total){
		$obj  = new toWords($Total);
  		$txtTotal = $obj->words;
  		$body .= '<table>';
		$body .= '<tr>';
	...
	function checkAddpage($datajsonT,$datajsonL){
		if ($datajsonT[0]->CheckLine==0) {
			...
			foreach ($datajsonL as $kl => $vl) {
				...
				if (strlen($vl->RemarkDescription) < 30) {
					$countRemarkrow = 1;
				}
			...
			if ($rowLines > 10) {
				if($rowLines < 28 && $countRemarkrow == 1){
					return 'INPAGE';
				}
				return 'OUTPAGE';
			}else{
...
if ($addpage=='OUTPAGE') {
	...
	$datagrandsig = txtGrandTotolSig($datajsonT,$datajsonL,$Total);
	$pdf->WriteHTML($datagrandsig);
}
